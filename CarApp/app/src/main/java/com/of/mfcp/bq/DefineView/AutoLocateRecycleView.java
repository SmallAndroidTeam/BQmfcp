package com.of.mfcp.bq.DefineView;
...
public class AutoLocateRecycleView extends RecyclerView {
    /**
    ...
    public AutoLocateRecycleView(Context context) {
        super(context);
    }

    public AutoLocateRecycleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public AutoLocateRecycleView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    /**
     * 设置初始化时选中的位置,该方法必须在{@link AutoLocateRecycleView#setAdapter(Adapter) }之前调用
     *
    ...
    /**
     * 设置每次显示多少个item,该方法必须在{@link AutoLocateRecycleView#setAdapter(Adapter) }之前调用
     *
